...
claimsFF <- rxImport(inData = inFile, outFile = "claimsFF.xdf", overwrite = T)
...
claimsSAS <- rxImport(inData = inFileSAS, outFile = xdfFileSAS, overwrite = TRUE)
rxGetInfo(claimsSAS, getVarInfo = TRUE)

# Importing SPSS Data
inFileSpss <- file.path(readPath, "claims.sav")
xdfFileSpss <- "claimsSpss.xdf"
claimsSpss <- rxImport(inData = inFileSpss, outFile = xdfFileSpss, overwrite = TRUE)
...
claimsNew <- rxImport(inFileAddVars, outFile = outfileTypeRelabeled,
    colInfo = colInfoList, overwrite = TRUE)
...
claimsDS <- rxImport(inFile, outFile = outfile,
    transforms = list(logcost = log(cost)), overwrite = TRUE)
rxGetInfo(claimsDS, getVarInfo = TRUE)
